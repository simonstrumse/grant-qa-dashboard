...
async function getGrants(searchParams: SearchParams) {
  ...
  // Map frontend sort keys to actual database columns
  // Use simpler field names that exist in the database
  let dbSortField = sortField

  switch (sortField) {
    case 'award_amount':
      // Sort by the raw text field (will group by text patterns)
      dbSortField = 'award_amount'
      break
    case 'application_deadline':
      // Sort by the raw deadline text field
      dbSortField = 'application_deadline'
      break
    case 'fields_count':
      // Sort by completeness score as proxy for field count
      dbSortField = 'completeness_score'
      break
    // grant_name and completeness_score don't need mapping
  }
